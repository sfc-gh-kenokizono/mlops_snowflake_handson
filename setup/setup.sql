...
📌 作成されるもの:
   - Git API統合 & Gitリポジトリ
   - データベース & スキーマ
   - CPUコンピュートプール（CPU_X64_M - コンテナランタイム）
   - ウェアハウス（SQLクエリ用 XS）
   - 顧客・注文データ（CSVからロード）
...
-- Notebook用コンピュートプール（CPUコンテナランタイム）
-- ウェアハウスより起動が早く、コスト効率が良い
CREATE COMPUTE POOL IF NOT EXISTS MLOPS_HOL_COMPUTE_POOL
  ...
  COMMENT = 'MLOps HOL - CPU Notebook Runtime';
...
-- Section 6: Experiment Viewer (Streamlit App)
...
SHOW COMPUTE POOLS LIKE 'MLOPS_HOL_COMPUTE_POOL';
...
📌 次のステップ:
   ...
   ※ ランタイム: コンテナランタイム（MLOPS_HOL_COMPUTE_POOL）
...
📌 実行順序:
   ...
   6. 06_EXPERIMENT_VIEWER   - Streamlitアプリで実験結果可視化

📌 ランタイム:
   - Notebook: CPUコンピュートプール（CPU_X64_M）
