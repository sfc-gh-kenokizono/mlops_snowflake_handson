...
-- Git API統合の作成（チーム共有アカウント用に_KEサフィックス）
...
-- Python/ML処理用ウェアハウス（Mサイズ）
CREATE WAREHOUSE IF NOT EXISTS MLOPS_HOL_PYTHON_WH
    WITH WAREHOUSE_SIZE = 'MEDIUM'
    AUTO_SUSPEND = 60
    AUTO_RESUME = TRUE
    INITIALLY_SUSPENDED = TRUE
    COMMENT = 'MLOps HOL - Python/ML Processing';
...
SELECT '✅ Step 2: ウェアハウス作成完了' AS STATUS;
...
-- Notebook・Streamlit作成権限
GRANT CREATE NOTEBOOK ON SCHEMA MLOPS_HOL_DB.NOTEBOOKS TO ROLE MLOPS_HOL_ROLE;
GRANT CREATE STREAMLIT ON SCHEMA MLOPS_HOL_DB.FEATURE_STORE TO ROLE MLOPS_HOL_ROLE;

-- Gitリポジトリ権限
GRANT READ ON GIT REPOSITORY MLOPS_HOL_DB.PUBLIC.MLOPS_HOL_REPO_KE TO ROLE MLOPS_HOL_ROLE;
...
SELECT 
    ...
    $4::VARCHAR AS REGION
FROM @MLOPS_HOL_DB.PUBLIC.MLOPS_HOL_REPO_KE/branches/main/data/customers.csv
...
SELECT 
    ...
    $5::VARCHAR AS STATUS
FROM @MLOPS_HOL_DB.PUBLIC.MLOPS_HOL_REPO_KE/branches/main/data/orders.csv
(FILE_FORMAT => CSV_FORMAT);
...
CREATE OR REPLACE NOTEBOOK MLOPS_HOL_DB.NOTEBOOKS."01_DATA_EXPLORATION"
  FROM '@MLOPS_HOL_DB.PUBLIC.MLOPS_HOL_REPO_KE/branches/main/notebooks/'
...
-- Section 6: Experiment Viewer（Streamlit参照用）
CREATE OR REPLACE NOTEBOOK MLOPS_HOL_DB.NOTEBOOKS."06_EXPERIMENT_VIEWER_APP"
  FROM '@MLOPS_HOL_DB.PUBLIC.MLOPS_HOL_REPO_KE/branches/main/notebooks/'
  MAIN_FILE = '06_experiment_viewer_app.ipynb'
  WAREHOUSE = MLOPS_HOL_PYTHON_WH
  QUERY_WAREHOUSE = MLOPS_HOL_SQL_WH;

ALTER NOTEBOOK MLOPS_HOL_DB.NOTEBOOKS."06_EXPERIMENT_VIEWER_APP" ADD LIVE VERSION FROM LAST;

SELECT '✅ Step 6: Notebook作成完了（6つ）' AS STATUS;
...
📌 実行順序:
   ...
   4. 04_EXPERIMENT_TRACKING - 複数実験の比較 + Model Registry連携
   5. 05_MODEL_REGISTRY      - モデル登録 + 本番推論
   6. 06_EXPERIMENT_VIEWER   - Streamlitアプリ作成（実験可視化）
