...
- Train F1: 訓練データでのスコア
- Test F1: テストデータでのスコア
- Gap: Train - Test（大きいと過学習の可能性）

DeepTreeモデルのGapが大きいのは、
深い木が訓練データに過剰適合しているためです。」
```

### 📊 UI確認ポイント（重要！）
- **AI & ML → Experiments** を開く
- 「CHURN_PREDICTION_EXPERIMENT」を選択
- 4つのRun（3実験 + BestModel）が表示されることを確認
- パラメータ・メトリクスの比較表を見せる
...
🆕 最良モデルはModel Registryにも自動登録されます。
実験Run ↔ モデルバージョン がリンクされ、
完全な追跡可能性（Traceability）を実現しています。」
```
...
Experiment Tracking経由で登録されているため、
実験Run ↔ モデルバージョン がリンクされています。」
```

#### 5.4 バージョン管理（v2追加）
```
「パラメータを改善してv2を追加作成します。
